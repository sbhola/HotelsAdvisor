!function (c) {
    ...
    function a() {
        var g = new Date();
        return f(g.getUTCFullYear(), g.getUTCMonth(), g.getUTCDate(), g.getUTCHours(), g.getUTCMinutes(), g.getUTCSeconds())
    }
    ...
    e.prototype = {constructor: e, _events: [], _attachEvents: function () {
        ...
        if (arguments && arguments.length && (typeof arguments[0] === "string" || arguments[0] instanceof Date)) {
            ...
            g = this.isInput ? this.element.prop("value") : this.element.data("date") || this.element.find("input").prop("value") || new Date()
        }
        if (!g) {
            g = new Date();
            h = false
        ...
        var y = new Date(this.viewDate), n = y.getUTCFullYear(), z = y.getUTCMonth(), h = y.getUTCDate(), v = y.getUTCHours(), q = y.getUTCMinutes(), r = this.startDate !== -Infinity ? this.startDate.getUTCFullYear() : -Infinity, w = this.startDate !== -Infinity ? this.startDate.getUTCMonth() : -Infinity, k = this.endDate !== Infinity ? this.endDate.getUTCFullYear() : Infinity, s = this.endDate !== Infinity ? this.endDate.getUTCMonth() : Infinity, l = (new f(this.date.getUTCFullYear(), this.date.getUTCMonth(), this.date.getUTCDate())).valueOf(), x = new Date();
        this.picker.find(".datetimepicker-days thead th:eq(1)").text(d[this.language].months[z] + " " + n);
        this.picker.find(".datetimepicker-hours thead th:eq(1)").text(h + " " + d[this.language].months[z] + " " + n);
        this.picker.find(".datetimepicker-minutes thead th:eq(1)").text(h + " " + d[this.language].months[z] + " " + n);
        this.picker.find("tfoot th.today").text(d[this.language].today).toggle(this.todayBtn !== false);
        this.updateNavArrows();
        this.fillMonths();
        var B = f(n, z - 1, 28, 0, 0, 0, 0), u = b.getDaysInMonth(B.getUTCFullYear(), B.getUTCMonth());
        B.setUTCDate(u);
        B.setUTCDate(u - (B.getUTCDay() - this.weekStart + 7) % 7);
        var g = new Date(B);
        ...
        while (B.valueOf() < g) {
            ...
            o = "";
            if (B.getUTCFullYear() < n || (B.getUTCFullYear() == n && B.getUTCMonth() < z)) {
                o += " old"
            } else {
                if (B.getUTCFullYear() > n || (B.getUTCFullYear() == n && B.getUTCMonth() > z)) {
                    o += " new"
            ...
            if ((B.valueOf() + 86400000) <= this.startDate || B.valueOf() > this.endDate || c.inArray(B.getUTCDay(), this.daysOfWeekDisabled) !== -1) {
                o += " disabled"
            }
            m.push('<td class="day' + o + '">' + B.getUTCDate() + "</td>");
        ...
        for (var t = 0; t < 24; t++) {
            var p = f(n, z, h, t);
            o = "";
            if ((p.valueOf() + 3600000) <= this.startDate || p.valueOf() > this.endDate) {
                o += " disabled"
            } else {
                if (v == t) {
                    o += " active"
            ...
            m.push('<span class="hour' + o + '">' + t + ":00</span>")
        }
        ...
        for (var t = 0; t < 60; t += this.minuteStep) {
            var p = f(n, z, h, v, t);
            o = "";
            if (p.valueOf() < this.startDate || p.valueOf() > this.endDate) {
                o += " disabled"
            } else {
                if (Math.floor(q / this.minuteStep) == Math.floor(t / this.minuteStep)) {
                    o += " active"
                }
            }
            m.push('<span class="minute' + o + '">' + v + ":" + (t < 10 ? "0" + t : t) + "</span>")
        }
        this.picker.find(".datetimepicker-minutes td").html(m.join(""));
        var C = this.date.getUTCFullYear();
        var j = this.picker.find(".datetimepicker-months").find("th:eq(1)").text(n).end().find("span").removeClass("active");
        if (C == n) {
        ...
        if (n == r) {
            j.slice(0, w).addClass("disabled")
        }
        if (n == k) {
            j.slice(s + 1).addClass("disabled")
        }
        m = "";
        ...
        n -= 1;
        for (var t = -1; t < 11; t++) {
            m += '<span class="year' + (t == -1 || t == 10 ? " old" : "") + (C == n ? " active" : "") + (n < r || n > k ? " disabled" : "") + '">' + n + "</span>";
            n += 1
        }
        A.html(m)
        ...
        k.stopPropagation();
        k.preventDefault();
        var l = c(k.target).closest("span, td, th");
        if (l.length == 1) {
            if (l.is(".disabled")) {
            ...
            switch (l[0].nodeName.toLowerCase()) {
                case"th":
                    switch (l[0].className) {
                        ...
                        case"today":
                            var h = new Date();
                            h = f(h.getFullYear(), h.getMonth(), h.getDate(), h.getHours(), h.getMinutes(), h.getSeconds());
                            this.viewMode = this.startViewMode;
                            this.showMode(0);
                            this._setDate(h);
                            break
                ...
                case"span":
                    if (!l.is(".disabled")) {
                        if (l.is(".month")) {
                            ...
                            var m = l.parent().find("span").index(l);
                            this.viewDate.setUTCMonth(m);
                            this.element.trigger({type: "changeMonth", date: this.viewDate})
                        } else {
                            if (l.is(".year")) {
                                this.viewDate.setUTCDate(1);
                                var n = parseInt(l.text(), 10) || 0;
                                this.viewDate.setUTCFullYear(n);
                                this.element.trigger({type: "changeYear", date: this.viewDate})
                            } else {
                                if (l.is(".hour")) {
                                    var p = parseInt(l.text(), 10) || 0;
                                    var n = this.viewDate.getUTCFullYear(), m = this.viewDate.getUTCMonth(), o = this.viewDate.getUTCDate(), i = this.viewDate.getUTCMinutes(), q = this.viewDate.getUTCSeconds();
                                    this._setDate(f(n, m, o, p, i, q, 0))
                                } else {
                                    if (l.is(".minute")) {
                                        var i = parseInt(l.text().substr(l.text().indexOf(":") + 1), 10) || 0;
                                        var n = this.viewDate.getUTCFullYear(), m = this.viewDate.getUTCMonth(), o = this.viewDate.getUTCDate(), p = this.viewDate.getUTCHours(), q = this.viewDate.getUTCSeconds();
                                        this._setDate(f(n, m, o, p, i, q, 0))
                                    }
                                }
                            }
    ...
    var b = {modes: [
        ...
        if (/^[-+]\d+[dmwy]([\s,]+[-+]\d+[dmwy])*$/.test(l)) {
            ...
            return f(l.getUTCFullYear(), l.getUTCMonth(), l.getUTCDate(), l.getUTCHours(), l.getUTCMinutes(), l.getUTCSeconds())
        }
        var m = l && l.match(this.nonpunctuation) || [], l = new Date(0, 0, 0, 0, 0, 0), q = {}, t = ["hh", "h", "ii", "i", "ss", "s", "yyyy", "yy", "M", "MM", "m", "mm", "d", "dd"], v = {hh: function (s, i) {
        ...
        var n;
        if (k == "standard") {
            n = {yy: g.getUTCFullYear().toString().substring(2), yyyy: g.getUTCFullYear(), m: g.getUTCMonth() + 1, M: d[o].monthsShort[g.getUTCMonth()], MM: d[o].months[g.getUTCMonth()], d: g.getUTCDate(), h: g.getUTCHours(), i: g.getUTCMinutes(), s: g.getUTCSeconds(), };
            n.hh = (n.h < 10 ? "0" : "") + n.h;
            n.ii = (n.i < 10 ? "0" : "") + n.i;
            n.ss = (n.s < 10 ? "0" : "") + n.s;
            n.dd = (n.d < 10 ? "0" : "") + n.d;
